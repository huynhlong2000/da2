...
function confirm(id) {
    Dialog.create({
        ...
        title: 'Xác nhận',
        color: 'primary',
        message: 'Xóa Tag này,ghi chú vẫn còn',
        cancel: true,
        persistent: true
    }).onOk(() => {
        try {
    ...
    <q-page class="flex flex-center">
        <q-card v-if="isEditing" flat bordered class="my-card">
            <q-card-section class="row items-center q-pb-none q-mb-md" vertical>
                <div class="text-h6">Chỉnh sửa tag:</div>
            </q-card-section>

            <q-card-section>
                <q-form @submit.prevent="handleSubmit">
                    <q-input filled v-model="tag.name" label="Thẻ tên" required lazy-rules
                        :rules="[val => val && val.length > 0 || 'Thẻ tên bắt buộc']" />
        ...
        <q-card v-else class="my-card q-mt-md" flat bordered>
            ...
            <q-card-actions>
                <q-btn color="primary" type="button" size="sm" @click="isEditing = true">Sửa</q-btn>
                <q-btn color="negative" type="button" size="sm" @click="confirm(tag.id)">Xóa</q-btn>
            </q-card-actions>
        </q-card>
