    ...
    <q-page class="flex flex-center">
        <q-card v-if="isEditing" flat bordered class="my-card">
            <q-card-section class="row items-center q-pb-none q-mb-md" vertical>
                <div class="text-h6">Chỉnh sửa</div>
            </q-card-section>

            <q-card-section>
                <q-form @submit.prevent="handleSubmit">
                    <q-input filled v-model="note.title" label="Tiêu đề" required lazy-rules
                        :rules="[val => val && val.length > 0 || 'Phải có tiêu đề']" />

                    <q-input filled v-model="note.content" type="textarea" required label="Nội dung" lazy-rules
                        :rules="[val => val && val.length > 0 || 'Phải có nội dung']" />

                    <div class="q-my-md"><label class="typo__label" v-if="notesStore.tags.length > 0">Gắn thẻ</label>
                        <multiselect 
                            class="q-my-md" 
                            v-model="note.tags" 
                            placeholder="Search" 
        ...
        <q-card v-else class="my-card q-mt-md" flat bordered>
            ...
            <q-card-actions>
                <q-btn color="primary" type="button" size="sm" @click="isEditing = true">Chỉnh sửa</q-btn>
                <q-btn color="negative" type="button" size="sm" @click="confirm">Xóa</q-btn>
            </q-card-actions>
        </q-card>
